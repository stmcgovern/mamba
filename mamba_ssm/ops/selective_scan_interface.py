...
try:
    ...
    custom_fwd = lambda f=None, **kw: f if f is not None else (lambda f: f)
...
try:
    ...
    if hasattr(torch.ops, "selective_scan") and hasattr(torch.ops.selective_scan, "fwd"):
        ...
        def _selective_scan_bwd_fake(u, delta, A, B, C, D, z, delta_bias, dout,
            ...
            dD = D.new_empty(D.shape) if D is not None else torch.empty(0, dtype=u.dtype, device=u.device)
            ddelta_bias = delta_bias.new_empty(delta_bias.shape) if delta_bias is not None else torch.empty(0, dtype=u.dtype, device=u.device)
            result = [du, ddelta, dA, dB, dC, dD, ddelta_bias]
            if z is not None:
                result.append(dz if dz is not None else z.new_empty(z.shape))
            if recompute_out_z:
                if out is not None:
                    result.append(out.new_empty(out.shape))
                elif z is not None:
                    result.append(z.new_empty(z.shape))
                else:
                    result.append(u.new_empty(u.shape))
            return result
    else:
        raise RuntimeError(
